...
public class UniversityNetworkController {
    ...
    public void doAddPerson() {
        try {
            ...
            if(id.trim().isEmpty()) {
                view.displayError("You must provide an ID.");
                return;
            }
            ...
            if(role.trim().isEmpty()) {
                view.displayError("You must provide a role.");
                return;
            }

            model.addPerson(new Person(Integer.parseInt(id), name, Enum.valueOf(Person.PersonRole.class, role)));
